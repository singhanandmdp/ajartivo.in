...
const AJARTIVO_PARTIAL_CACHE_VERSION = "20260605-1";
...
function getLocalRouteFilePath(pathname) {
    ...
    const routeMap = {
        ...
        "/pages/header": "/pages/header.html",
        "/pages/footer": "/pages/footer.html",
        "/pages/sidebar": "/pages/sidebar.html",
...
function resolveSiteUrl(path) {
    ...
    if (isLocalDevelopmentHost()) {
        if (cleanPath.startsWith("/product/")) {
            ...
            if (slug) {
                const productQueryPath = `/product.html?slug=${encodeURIComponent(slug)}`;
                if (suffix.startsWith("?")) {
                    return joinBasePathWithTarget(basePath, `${productQueryPath}&${suffix.slice(1)}`);
                }

                return joinBasePathWithTarget(basePath, productQueryPath) + suffix;
            }
        }
...
function rememberClickedProductLink(href) {
    try {
        const url = new URL(String(href || ""), window.location.href);
        const slug = cleanText(url.searchParams.get("slug")) || extractProductSlug(url.pathname);
...
function extractProductSlug(pathname) {
    const cleanPath = collapseDuplicateLeadingSegment(stripHtmlExtensionFromPath(String(pathname || "")));
    if (!cleanPath.startsWith("/product/")) {
        return "";
    }

    return cleanText(cleanPath.slice("/product/".length).split("/")[0]);
}
