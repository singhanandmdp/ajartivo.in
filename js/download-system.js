(function () {
    ...
    const resolveUrl = typeof window.AjArtivoResolveUrl === "function"
        ...
        : function (path) {
            ...
            const normalizedPath = String(suffixMatch ? suffixMatch[1] : input)
                .replace(/\/index\.html(?=([?#]|$))/i, "/")
                .replace(/\.html(?=([?#]|$))/i, "");
            const suffix = suffixMatch && suffixMatch[2] ? suffixMatch[2] : "";

            if (normalizedPath.startsWith("/product/")) {
                const slug = normalizedPath.slice("/product/".length).replace(/\/+$/, "");
                if (slug) {
    ...
    const requestedId = cleanText(params.get("id"));
    const requestedSlug = cleanText(params.get("slug")) || extractPathSlug();
    const hasExplicitRequest = Boolean(requestedId || requestedSlug);
    const designId = requestedId || (!hasExplicitRequest ? rememberedProductId : "");
    const designSlug = requestedSlug || (!hasExplicitRequest ? readRememberedProductSlug() : "");
    ...
    async function initPage() {
        ...
        try {
            ...
            if (!designId && !designSlug) {
                console.error("Product lookup skipped because the URL is missing both slug and id.", {
            ...
            if (!currentDesign) {
                console.warn("Product lookup fell back to a placeholder design.", {
                    requestedId: designId,
                    requestedSlug: designSlug
                });
    ...
    function renderDesign(product) {
        ...
        try {
            ...
            console.warn("Product SEO update failed:", error);
        }

        try {
            renderFeatures(product, type);
        } catch (error) {
            console.warn("Product feature render failed:", error);
        }

        try {
            renderGalleryCaption(product, type);
        } catch (error) {
            console.warn("Product gallery caption render failed:", error);
        }

        try {
            renderThumbnails(getDesignImages(product), title);
        } catch (error) {
            console.warn("Product thumbnail render failed:", error);
        }
        ...
        try {
            ...
            console.warn("Product access UI update failed:", error);
        }
    }
    ...
    function renderGalleryCaption(product, type, accessState) {
        ...
        const fallback = isFreeDesign(product)
            ? "Simple product file with quick login-based download access."
            : "Premium product file with secure access and quick delivery.";
    ...
    function updateProductJsonLd(product, data) {
        ...
        const offer = {
            ...
            "availability": resolveOfferAvailability(product)
        };
        ...
        const schema = {
            ...
            "image": image,
            "description": description,
            "category": cleanText(product && product.category) || undefined,
            "brand": {
                "@type": "Brand",
                "name": "AJartivo"
            },
    ...
    function resolveOfferAvailability(product) {
        if (!product) {
            return "https://schema.org/PreOrder";
        }

        if (isFreeDesign(product) || hasDownloadAccess(product) || product.download_enabled === true) {
    ...
    function getDesignSlug(product) {
        ...
        const explicitSlug = cleanText(product && product.slug);
        if (explicitSlug) {
